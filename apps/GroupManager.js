...
export class GroupManager extends plugin {
  ...
  async handleMuteAction(e) {
    ...
    const isMute = e.msg.startsWith("禁言")

    if (isMute) {
      let { targetQQ, duration, unit } = this.parseMuteCommand(e.msg)
      ...
      const targetQQ = e.msg.replace(/解禁/g, "").trim().replace("@", "") || e.at
      if (!targetQQ) return false
  ...
  async kickMember(e) {
    ...
    const isBlacklist = e.msg.startsWith("踢黑")
    const command = isBlacklist ? "踢黑" : "踢"
    const targetQQ = e.msg.replace(command, "").trim().replace("@", "") || e.at
  ...
  async handleEssenceMessage(e) {
    ...
    const action = e.msg.includes("设为精华") ? "set" : "remove"
  ...
  async managePermission(e) {
    ...
    const actionText = isRemove ? "取消赋权" : "赋权"
