...
export class UserProfilePlugin extends plugin {
    ...
    async generateUserProfile(e) {
        ...
        if (messages && messages.length > 0) {
            const formattedLines = await Promise.all(messages.map(async (chat) => {
                ...
                const contentParts = await Promise.all(chat.message.map(async (part) => {
                    if (part.type === 'text') {
                        return part.text;
                    }
                    if (part.type === 'at') {
                        ...
                        try {
                            const member = e.bot.pickMember(e.group_id, part.qq);
