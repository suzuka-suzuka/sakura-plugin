...
${rawChatHistory}`
  ...
  async mimicUser(e) {
    ...
    try {
      ...
      const aiPrompt = `你的任务是深度模仿用户【${nickname}】的说话风格。请仔细分析下面提供的聊天记录，然后生成三句符合他/她语言习惯和话题倾向的回复。你的目标是让这些回复看起来就像是【${nickname}】本人说出来的一样。
**模仿要点：**
1.  **核心口癖和短语：** 识别并使用他/她最常说的词、口头禅或短语，可以适当引用聊天记录中的原话或短语来增强模仿度。
2.  **语气和情感：** 模仿他/她通常表现出的情绪，是活泼、冷静、还是讽刺？
3.  **标点和符号：** 注意他/她使用标点符号的习惯，比如是否常用“~”、“...”或者表情符号。
4.  **句子结构：** 是长句还是短句？是复杂的从句还是简单的陈述？
5.  **互动模式：** 他/她是如何回应别人的？是喜欢提问，还是经常发表看法？
6.  **话题倾向：** 识别用户常聊的话题（如：游戏、动漫、技术），并让生成的回复内容与这些话题相关。
**要求：**
-   **不要**在回复中解释你的模仿行为或提及“模仿”二字。
-   **不要**偏离【${nickname}】的人设（OOC）。
-   生成**三句**独立的回复，每句占一行，用换行符隔开。
**分析材料：**
以下是该用户的发言记录：
${rawChatHistory}`
