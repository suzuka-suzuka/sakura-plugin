function compactErrorMessage(message) {
  return String(message || "")
    .replace(/\s+/g, " ")
    .replace(/(api[_-]?key|authorization|token)\s*[:=]\s*[^,\s]+/gi, "$1=***")
    .trim();
}
...
export function formatGrokUserError(error, kind = "request") {
  ...
  if (
    ...
    return "本地 Grok API Key 不对，请检查 CliProxyMedia.apiKey。";
  }
  ...
  if (
    ...
    return "提示词或参考图可能触发了 Grok 的内容审核，请换个说法或换一张参考图再试。";
  }
  ...
  if (
    ...
    return "Grok 没有返回图片，可能是提示词被拦截、额度不足，或 API 网关返回异常。";
  }
  ...
  if (
    ...
    return "Grok 没有返回视频结果，可能是提示词被拦截、排队失败，或 API 网关返回异常。";
  }
