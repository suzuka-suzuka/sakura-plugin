...
async function _getOpenAIResponse(
  ...
  try {
    ...
    if (queryParts && queryParts.length > 0) {
      const openAICompatibleParts = queryParts.map(part => {
...
async function _getGeminiResponse(
  ...
  const isVertex = channel.channelType === "vertex"
  if (
    !channel ||
    (!isVertex && !channel.api) ||
    (!isVertex && Array.isArray(channel.api) && channel.api.length === 0) ||
    (!isVertex && typeof channel.api === "string" && !channel.api.trim()) ||
    !channel.model
  ) {
    const errorMessage = "无效或不完整的渠道配置。"
    logger.error(errorMessage)
...
export async function getAI(
  ...
  let result
  if (channelType === "gemini" || channelType === "vertex") {
