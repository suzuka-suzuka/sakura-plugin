...
import { formatGrokUserError } from "./grokErrorMessages.js";
...
const OPENAI_NATIVE_WEB_SEARCH_TOOL = {
  type: "web_search",
...
async function _getOpenAIResponse(
  ...
  try {
    ...
    if (channel.nativeWebSearch === true) {
      allOpenAITools.push(OPENAI_NATIVE_WEB_SEARCH_TOOL);
    }
    if (enableTools) {
...
async function _getGrokResponse(
  ...
  if (!channel || !channel.model) {
    const errorMessage = "Grok 渠道配置不完整，请检查模型和渠道设置。";
    logger.error(errorMessage);
    return errorMessage;
  }

  if (!channel.sso && !channel.supersso) {
    const errorMessage = "Grok 登录凭据没配好，请在 grok 渠道里填写 sso 或 supersso。";
    logger.error(errorMessage);
    return errorMessage;
  }
  ...
  try {
    let messages = [];
    const grokSessionKey = e?._grokSessionKey || "";
    const effectiveHistoryContents = grokSessionKey ? [] : historyContents;
    ...
    if (effectiveHistoryContents.length > 0) {
      for (const item of effectiveHistoryContents) {
        if (item.role === "user" || item.role === "model") {
          const role = item.role === "model" ? "assistant" : "user";
          const textParts = item.parts.filter((p) => p.text).map((p) => p.text);
          const content = textParts.join("");
    ...
    if (
      ...
      const errorMessage = "没有收到有效的问题内容，暂时没法向 Grok 发起请求。";
      logger.warn(errorMessage);
      return errorMessage;
    }
    ...
    const errorMessage = `Grok 请求失败：${formatGrokUserError(error, "text")}`;
    logger.error(errorMessage, error);
    return errorMessage;
  }
}
...
export async function getAI(
  ...
  if (!channelConfig) {
    if (!isFinalFallback && channelName !== defaultChannelName && defaultChannelName) {
      ...
      return getAI(defaultChannelName, e, queryParts, presetPrompt, enableGroupContext, enableTools, historyContents, -1);
    }
  ...
  if (typeof result === "string") {
    ...
    if (!isFinalFallback && _retryCount < maxRetries) {
      ...
      return getAI(channelName, e, queryParts, presetPrompt, enableGroupContext, enableTools, historyContents, _retryCount + 1);
    }
