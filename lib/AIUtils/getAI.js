...
async function _getOpenAIResponse(
  ...
  try {
    ...
    if (historyContents.length > 0) {
      for (const item of historyContents) {
        if (item.role === "user" || item.role === "model") {
          ...
          const reasoningPart = item.parts.find(p => p.reasoning_content)
          let message = { role }
          if (content) message.content = content
          if (tool_calls.length > 0) message.tool_calls = tool_calls
          if (reasoningPart) message.reasoning_content = reasoningPart.reasoning_content
          if (message.content || message.reasoning_content || (message.tool_calls && message.tool_calls.length > 0)) {
    ...
    return { text: extractedText, functionCalls: functionCalls, reasoning_content: reasoningContent }
  } catch (error) {
