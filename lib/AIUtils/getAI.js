...
async function _getOpenAIResponse(
  ...
  try {
    ...
    let allOpenAITools = [];
    if (enableTools) {
      const toolsSchema = await getToolsSchema(e);
      ...
      try {
        const mcpTools = await mcpManager.getOpenAITools(Boolean(e?.isMaster), vectorContext);
...
async function _getGeminiResponse(
  ...
  try {
    ...
    if (enableTools) {
      ...
      try {
        const mcpTools = await mcpManager.listTools(Boolean(e?.isMaster), vectorContext);
        if (mcpTools && mcpTools.length > 0) {
          const mcpDeclarations = mcpTools.map(tool => {
            const params = JSON.parse(JSON.stringify(tool.inputSchema || { type: "object", properties: {} }));
            const upperParams = JSON.parse(JSON.stringify(params));
            const upperType = (obj) => {
              if (obj && typeof obj === "object") {
                if (obj.type && typeof obj.type === "string") obj.type = obj.type.toUpperCase();
                delete obj.exclusiveMaximum;
                delete obj.exclusiveMinimum;
                Object.values(obj).forEach(v => upperType(v));
              }
            };
            upperType(upperParams);
            return {
              name: tool.name,
              description: tool.description || "",
              parameters: upperParams,
            };
          });
...
export async function getAI(
  ...
  if (!channelConfig) {
    ...
    if (channelName !== defaultChannelName) {
      ...
      return getAI(defaultChannelName, e, queryParts, presetPrompt, enableGroupContext, enableTools, historyContents, null, true);
    }
    logger.warn(`渠道 "${channelName}" 未找到，10 秒后重试...`);
    await new Promise((r) => setTimeout(r, 10000));
    return getAI(channelName, e, queryParts, presetPrompt, enableGroupContext, enableTools, historyContents, null, true);
  }
  ...
  if (typeof result === "string" && !_retrying) {
    if (channelName !== defaultChannelName) {
      logger.info(`尝试使用 '${defaultChannelName}' 渠道进行重试...`);
      return getAI(defaultChannelName, e, queryParts, presetPrompt, enableGroupContext, enableTools, historyContents, null, true);
    }
    logger.info(`默认渠道出错，等待 10 秒后重试...`);
    await new Promise((r) => setTimeout(r, 10000));
    return getAI(channelName, e, queryParts, presetPrompt, enableGroupContext, enableTools, historyContents, null, true);
  }
  return result;
