...
function normalizeOpenAIMessageImages(message) {
  const images = [];
  ...
  for (const image of message?.images || []) {
    pushImageUrl(image?.url || image?.image_url?.url);
  }

  for (const part of message?.content || []) {
    if (part?.type === "image_url") {
      pushImageUrl(part.image_url?.url || part.url);
    }
  }
...
async function sendOpenAIMessageImages(e, images = []) {
  if (!e?.reply || !Array.isArray(images) || images.length === 0) {
    return;
  }
  ...
  for (const image of images) {
    try {
      ...
      const imageInput = dataUrlMatch
        ? Buffer.from(dataUrlMatch[1], "base64")
        : url;
      await e.reply(segment.image(imageInput));
    } catch (error) {
      logger.warn(`[OpenAI] failed to send generated image: ${error.message}`);
    }
  }
}
...
async function _getOpenAIResponse(
  ...
  try {
    ...
    let allOpenAITools = [...OPENAI_NATIVE_TOOLS];
    if (enableTools) {
    ...
    const extractedImages = normalizeOpenAIMessageImages(message);
    const toolCallsArr = message?.tool_calls || [];
    await sendOpenAIMessageImages(e, extractedImages);

    if (!extractedText && extractedImages.length === 0 && toolCallsArr.length === 0) {
