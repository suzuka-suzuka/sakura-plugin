...
async function _getOpenAIResponse(
  ...
  try {
    ...
    if (queryParts && queryParts.length > 0) {
      messages.push({ role: "user", content: queryParts })
    }
...
async function _getGeminiResponse(
  ...
  try {
    ...
    const result = await ai.models.generateContent(geminiRequestBody)
...
async function _getVertexAIResponse(
  ...
  try {
    ...
    const result = await ai.models.generateContent(vertexRequestBody)
    const response = result.response
