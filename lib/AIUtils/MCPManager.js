...
class MCPManager {
    ...
    isServerEnabled(serverId) {
        ...
        if (Object.prototype.hasOwnProperty.call(enabledMCPs, serverId)) {
            return Boolean(enabledMCPs[serverId]);
        }
        // 配置缺失时默认启用
        return true;
    }
    ...
    async connectAll() {
        ...
        this._connecting = (async () => {
            ...
            // 过滤被禁用的MCP服务器
            const enabledServers = SERVERS.filter(serverConfig => {
            ...
            // 仅连接当前启用且尚未连接的 server（支持运行时从“关”切到“开”）
            const serversToStart = enabledServers.filter(serverConfig => !this.clients.has(serverConfig.id));
    ...
    async listTools(isMaster = false, vectorContext = null) {
        ...
        const eligibleServers = SERVERS.filter(s =>
            this.clientTools.has(s.id) && this.isServerEnabled(s.id) && !(s.masterOnly && !isMaster)
        );
