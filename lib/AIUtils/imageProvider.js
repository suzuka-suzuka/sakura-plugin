...
function getConversationChannelName(imageConfig) {
  return imageConfig.name || imageConfig.imageChannel || "";
}
...
function getOpenAIConversationSessionImage(session, channelName, model) {
  if (
  ...
  const imageData = session.lastImage;
  ...
  const mimeType =
    imageData.mimeType || getMimeTypeFromDataUrl(base64Data) || "image/png";
  const buffer = Buffer.from(normalizeBase64Data(base64Data), "base64");
  if (buffer.length === 0) {
    return null;
  }
  ...
  return {
    ...
    fileName:
      imageData.fileName ||
      `previous-result.${getFileExtension(mimeType)}`,
  };
}

function buildOpenAIConversationSession(
  imageBuffers,
  imageResults,
  channelName,
  model
) {
  const firstImageResult = imageResults?.[0] || null;
  const firstImageBuffer = imageBuffers?.[0] || null;
  ...
  return {
    ...
    model,
    lastImage: {
      base64: firstImageResult
        ? normalizeBase64Data(firstImageResult)
        : firstImageBuffer.toString("base64"),
      mimeType:
        (firstImageResult && getMimeTypeFromDataUrl(firstImageResult)) ||
        "image/png",
...
async function callOpenAIResponsesImage(
  ...
  const channelName = getConversationChannelName(imageConfig);
  const previousImage = getOpenAIConversationSessionImage(
    session,
    channelName,
    model
  );
  const conversationImages = previousImage
    ? [previousImage, ...sourceImages]
    : sourceImages;
  const tool = {
    type: "image_generation",
    action: conversationImages.length > 0 ? "edit" : "auto",
  ...
  const request = {
    model,
    input: buildOpenAIResponsesInput(promptText, conversationImages),
  ...
  const nextSession =
    imageBuffers.length > 0
      ? buildOpenAIConversationSession(
          imageBuffers,
          imageResults,
          channelName,
          model
...
export async function continueImageConversationWithProvider(
  ...
  if (provider === "openai") {
    ...
    if (isGrokImageModel(resolvedConfig.model)) {
      const channelName = getConversationChannelName(resolvedConfig);
      const previousImage = getOpenAIConversationSessionImage(
        session,
        channelName,
      ...
      const conversationImages = previousImage
        ? [previousImage, ...sourceImages]
        : sourceImages;
      const imageBuffers = await callOpenAIImage(
        resolvedConfig,
        promptText,
        conversationImages,
        options
