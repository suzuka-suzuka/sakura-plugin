...
function buildRelationshipGraphData(stats, analysis) {
  const nodes = (stats.topMembers || []).slice(0, 8).map((member) => ({
    ...
    name: member.name,
    interactionCount: Number(member.sentInteractions || 0)
      + Number(member.receivedInteractions || 0),
  }));
...
function renderRelationshipGraph(stats, analysis) {
  ...
  const maxExplicit = Math.max(1, ...edges.map((edge) => edge.explicitCount));
  const edgeLines = edges.map((edge) => {
    const start = positions.get(edge.userAId);
    const end = positions.get(edge.userBId);
    if (!start || !end) return "";
    const strokeWidth = edge.explicitCount > 0
      ? 2.5 + edge.explicitCount / maxExplicit * 5
      : 3;
    const className = edge.explicitCount > 0 ? "explicit-edge" : "context-edge";
    const label = edge.explicitCount > 0 ? `${edge.explicitCount}次` : "语境";
    return `<g>
      <line class="relation-line ${className}" x1="${start.x}" y1="${start.y}" x2="${end.x}" y2="${end.y}" style="stroke-width:${strokeWidth}px" />
      <text class="relation-line-label" x="${(start.x + end.x) / 2}" y="${(start.y + end.y) / 2 - 7}">${escapeHtml(label)}</text>
    </g>`;
  }).join("");
  const nodeHtml = nodes.map((node) => {
    ...
    return `<div class="relation-node" style="left:${position.x}px;top:${position.y}px">
      ...
      <span>显式 ${formatNumber(node.interactionCount)}</span>
    </div>`;
  }).join("");
  ...
  const contextHtml = contextualRelations.length
    ? `<div class="context-relations">${contextualRelations.map((relation) => `
        <div class="context-relation-card">
          ...
          <small>证据 ${relation.evidenceRefs.map(escapeHtml).join(" · ")}</small>
        </div>`).join("")}</div>`
    : "";
...
<head>
  ...
  <style>
    * { box-sizing: border-box; }
    ...
    .relation-line { fill: none; opacity: 0.78; }
    .explicit-edge { stroke: var(--blue); }
    .context-edge { stroke: var(--mauve); stroke-dasharray: 11 8; }
    .relation-line-label { fill: #bac2de; font-size: 15px; text-anchor: middle; paint-order: stroke; stroke: #181825; stroke-width: 5px; }
    .relation-node { position: absolute; width: 164px; min-height: 68px; padding: 10px 12px; transform: translate(-50%, -50%); border-radius: 14px; border: 1px solid rgba(203, 166, 247, 0.34); background: rgba(49, 50, 68, 0.96); text-align: center; box-shadow: 0 8px 22px rgba(0, 0, 0, 0.24); }
    .relation-node b { display: block; color: #f0e5fb; font-size: 19px; overflow: hidden; text-overflow: ellipsis; white-space: nowrap; }
    .relation-node span { display: block; margin-top: 4px; color: var(--muted); font-size: 15px; }
    ...
    .context-relation-card p { margin: 8px 0; color: #bac2de; font-size: 18px; line-height: 1.55; }
    .context-relation-card small { color: #7f849c; font-size: 14px; }
  ...
  <main id="capture-area">
    ...
    <section class="section">
      <div class="section-head">
        ...
        <div class="section-note">实线为 @/回复，虚线为经消息引用验证的上下文交流</div>
      </div>
