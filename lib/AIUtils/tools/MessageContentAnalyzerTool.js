...
const VERTEX_ADC_DIR = path.join(plugindata, "vertex-adc");
...
function pickApiKey(channel) {
  let apiKeys = splitApiKeys(channel?.api);
  ...
  if (Array.isArray(apiKeys) && apiKeys.length > 0) {
    ...
    logger.info(`Channel [${channelName}] using API key #${currentIndex + 1}`);
    return apiKey;
  }
...
function getVertexAdcFilePath(config = {}) {
  ...
  if (fs.existsSync(VERTEX_ADC_DIR)) {
    ...
    if (files.length > 0) {
      return path.join(VERTEX_ADC_DIR, files[0]);
    }
  }
...
function readJsonFile(filePath) {
  ...
  try {
    ...
    logger.warn(`[MessageContentAnalyzer] Failed to read JSON credentials: ${error.message}`);
    return null;
  }
}

function getVertexAdcConfig(config = {}) {
...
function createGeminiClient(config) {
  const isVertex = config.vertex === true;
  ...
  if (isVertex) {
    ...
    if (apiKey) {
      ...
      if (!adcConfig) {
        throw new Error("Vertex Gemini channel requires api, or ADC credentials with project_id.");
      }
      process.env.GOOGLE_APPLICATION_CREDENTIALS = adcConfig.filePath;
      options.project = adcConfig.project;
      options.location = adcConfig.location;
    }
  } else {
    if (!apiKey) {
      throw new Error("Invalid API key in channel configuration.");
    }
    options.apiKey = apiKey;
  }
...
function createStorageClient(config = {}) {
  ...
  const credentials = readJsonFile(keyFilename);
  const projectId =
    config.gcsProjectId ||
    config.project ||
    config.vertexProject ||
...
export class MessageContentAnalyzerTool extends AbstractTool {
  ...
  description =
    "通过seq或url(仅限其一)分析内容，或通过seq下载文件/撤回/加精/取消加精消息";
  ...
  async processVideo(videoTarget, query, e) {
    ...
    const Config = channelsConfig?.gemini?.find(
      (c) => c.name === aiConfig.toolschannel
    );

    if (!Config || !Config.model) {
      throw new Error(
        "配置错误：未在 'gemini' 配置中找到有效配置或缺少 model。"
      );
    }
