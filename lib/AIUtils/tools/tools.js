...
export function getToolsSchema(e) {
  let hasPermission = false
  if (e) {
    if (e.group_id && e.user_id) {
      const permInfo = PermissionManager.getUserPermissionInfo(e.group_id, e.user_id)
      hasPermission = permInfo.hasPermission
    } else if (e.user_id && PermissionManager.masterQQs.includes(e.user_id)) {
      hasPermission = true
    }
  }

  const filteredTools = availableTools.filter(tool => {
    if (tool.name === "ImageGenerator") {
      return hasPermission
    }
    return true
  })

  return filteredTools.map(tool => tool.function())
}
