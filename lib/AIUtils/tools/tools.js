...
const FILE_TOOLS = new Set(["Search", "SearchFilesText", "RunCommand", "RunPython"]);
...
function buildResultNews(toolName, toolArgs, result) {
  ...
  switch (toolName) {
    // ── 本地工具 ──────────────────────────────────────────
    case "Search":
      return `✅ ${toolArgs.query || '搜索'} 的搜索结果`;
    case "SearchFilesText":
...
async function sendToolVisual(e, toolName, toolArgs, result) {
  try {
    ...
    const searchNews = `${toolArgs?.query || '搜索'}的搜索结果`;
    const news = toolName === 'Search'
      ? [{ text: searchNews }]
      : [{ text: actionNews }, { text: resultNews }];
    ...
    if (MARKDOWN_TOOLS.has(toolName)) {
      ...
      await e.sendForwardMsg(nodes, {
        source: '工具执行',
        prompt: toolName === 'Search' ? searchNews : actionNews,
        news,
      });
    }
  } catch (err) {
