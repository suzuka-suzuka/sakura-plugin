...
import { downloadImage, checkAndFlipImage } from '../../ImageUtils/ImageUtils.js';
...
export class IllustrationTool extends AbstractTool {
    ...
    func = async function (opts, e) {
        ...
        try {
            ...
            const initialImageBuffer = await downloadImage(imageUrl);
            if (!initialImageBuffer) {
                return "图片下载失败，链接可能已失效或网络不稳定。";
            }

            let sendResult = await e.reply(segment.image(initialImageBuffer));

            const { success, processedBuffer } = await checkAndFlipImage(initialImageBuffer, sendResult);

            if (!success) {
                await e.reply('图片发送失败，正在尝试翻转后重发...');
                sendResult = await e.reply(segment.image(processedBuffer));
            }
			...
			if (isR18) {
                setTimeout(() => {
                    if (sendResult?.message_id) {
                        ...
                        target.recallMsg(sendResult.message_id);
                    }
                }, 10000); 
            }
