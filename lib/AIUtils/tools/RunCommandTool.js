...
export class RunCommandTool extends AbstractTool {
    name = "RunCommand"
    description = `在项目目录下执行命令行命令。当前系统: ${IS_WINDOWS ? "Windows" : process.platform}。工作目录: ${ALLOWED_ROOT}。支持任意合法命令，不允许破坏性操作。`
    parameters = {
        properties: {
            command: {
                type: "string",
            ...
            timeout: {
                type: "number",
                description: `超时秒数，默认 ${DEFAULT_TIMEOUT / 1000} 秒，最大 ${MAX_TIMEOUT / 1000} 秒。执行文件下载等耗时操作时建议设为 120~300。`,
            },
        },
        required: ["command"],
    }
