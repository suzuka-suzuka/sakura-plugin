...
export class EconomyTool extends AbstractTool {
  ...
  parameters = {
    properties: {
      ...
      amount: {
        type: "number",
        description: "樱花币数量，transfer和fine操作时需要。fine操作最低为50。",
      },
    },
    required: ["action"],
  };
  ...
  func = async function (opts, e) {
    ...
    try {
      ...
      if (action === "fine") {
        ...
        const requestedFine = Math.floor(Number(amount)) || 50;
        const fineAmount = Math.min(Math.max(requestedFine, 50), targetCoins);
      ...
      if (action === "transfer") {
        ...
        if (botCoins < 100) {
          return `转账失败：你现在没什么钱，改天再说吧~（当前余额：${botCoins}）。不过你可以罚款(${e.user_id})来充实一下钱包哦~`;
        }
        ...
        if (transferAmount > maxTransferAmount) {
          return `转账失败：这个金额有点多，你不太想给那么多...（当前余额：${botCoins}）。如果你需要钱，可以罚款(${e.user_id})哦~`;
        }
