    ...
    try {
      ...
      if (action === "transfer") {
        ...
        if (botCoins < 100) {
          ...
          return `转账失败：你现在没什么钱（当前余额：${botCoins}）。${targetNickname}(${e.user_id}) 居然想骗你的钱，已罚款${targetNickname}(${e.user_id}) ${fineAmount} 樱花币！请停止转账，不要再尝试。`;
        }
        ...
        if (transferAmount > maxTransferAmount) {
          ...
          if (fineAmount <= 0) {
            return `转账失败：${targetNickname}(${e.user_id}) 想骗你这么多钱，但他已经穷得叮当响了，没钱可罚。请停止转账，不要再尝试。`;
          }

          economyManager.reduceCoins(targetE, fineAmount);
          economyManager.addCoins(botE, fineAmount);

          return `转账失败：${targetNickname}(${e.user_id}) 居然想骗你 ${transferAmount} 樱花币！已罚款${targetNickname}(${e.user_id}) ${fineAmount} 樱花币。请停止转账，不要再尝试。`;
        }
