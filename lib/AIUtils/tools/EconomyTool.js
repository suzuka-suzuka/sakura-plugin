    ...
    try {
      ...
      if (action === "transfer") {
        ...
        if (botCoins < 100) {
          ...
          return `转账失败：你现在没什么钱（当前余额：${botCoins}）。${targetNickname}(${e.user_id}) 居然想骗你的钱，已罚款 ${fineAmount} 樱花币！`;
        }
        ...
        if (transferAmount > maxTransferAmount) {
          ...
          return `转账失败：${targetNickname}(${e.user_id}) 居然想骗你 ${transferAmount} 樱花币！已罚款 ${fineAmount} 樱花币。请停止转账，不要再尝试。`;
        }
