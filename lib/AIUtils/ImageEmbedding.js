...
function getToolsRouteConfig() {
  const aiConfig = Setting.getConfig("AI");
  const toolsRoute = aiConfig?.toolsRoute;

  if (!toolsRoute) {
    throw new Error("未配置 toolsRoute");
  }

  const resolved = resolveRouteTarget(toolsRoute);
  if (!resolved || resolved.provider.protocol !== "gemini") {
    throw new Error(`工具路由 ${toolsRoute} 必须包含可用的 Gemini 目标`);
  }
...
class ImageEmbeddingManager {
  ...
  createClient() {
    const channel = getToolsRouteConfig();
    return createGeminiClient(channel);
  }
