...
class ImageEmbeddingManager {
  ...
  getVertexAdcConfig() {
    const filePath = this.getVertexAdcFilePath();
    ...
    try {
      ...
      if (!project) {
        logger.warn(`[ImageEmbedding] Vertex ADC is missing project_id: ${filePath}`);
        return null;
      }

      return { filePath, project, location };
    } catch (error) {
      logger.warn(`[ImageEmbedding] Failed to read Vertex ADC: ${error.message}`);
      return null;
    }
  }
