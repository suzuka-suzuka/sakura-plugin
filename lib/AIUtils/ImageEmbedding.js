...
class ImageEmbeddingManager {
  ...
  async generateEmbedding(text, taskType = "SEMANTIC_SIMILARITY") {
    try {
      ...
      logger.error(`生成向量失败: ${error.message}`);
      throw error;
    }
  }
  ...
  async cleanupOrphanedIndexes() {
    ...
    for (const row of rows) {
      if (!row.file_path || !fs.existsSync(row.file_path)) {
        ...
        logger.warn(`[表情向量] 发现孤儿索引: ${row.id} - ${row.description?.substring(0, 30)}...`);
      }
    }

    if (orphanedIds.length === 0) {
      return { cleaned: 0, total: rows.length };
    }

    let cleaned = 0;
    for (const id of orphanedIds) {
      try {
        await this.deleteImage(id); // Handles both Index and DB
        cleaned++;
      } catch (error) {
    ...
    logger.mark(`[表情向量] 孤儿索引清理完成: 清理 ${cleaned} 个, 剩余 ${rows.length - cleaned} 个`);

    return { cleaned, total: rows.length - cleaned };
  }
...
export async function describeImage({
  ...
  const queryParts = [
    {
      text: `提取这张表情包/图片的核心关键词（内容、情感、氛围等）。只输出关键词，多个关键词用空格分隔，不要连贯的句子，不要使用Markdown格式，不要标点符号，不要开场白。`,
    },
    {
      inlineData: {
