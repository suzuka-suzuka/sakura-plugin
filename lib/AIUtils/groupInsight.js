...
const CACHE_KEY_PREFIX = "sakura:group-insight:v3";
...
function normalizeTranscriptContent(record, nameContext = {}) {
  const content = replaceGroupMemberReferences(
    record?.content || record?.textContent || "",
    record,
    nameContext
  );
  return content
...
function formatTranscriptLine(record, reference, content) {
  ...
  return `${reference} | ${time} | ${normalizeTranscriptField(record.senderName || record.userId)} `
    + `(QQ:${normalizeTranscriptField(record.userId)}, ${role}) | ${content}`;
}
...
export function buildGroupInsightAIInput(
  ...
  const nameContext = {
    memberNames: memberNames instanceof Map
      ? memberNames
      : buildGroupMemberNameMap(records),
    messageRecords: buildGroupMessageRecordMap(records),
  };
  const getContent = (record) => normalizeTranscriptContent(record, nameContext);
  const candidates = (Array.isArray(records) ? records : [])
    .filter((record) => getContent(record))
  ...
  while (selected.length > 0) {
    ...
    const textLength = lines.reduce((sum, item) => {
      return sum + formatTranscriptLine(
        item.record,
        item.reference,
        item.content
      ).length + 1;
...
- overview、mood、topics、金句理由、成员称号和关系理由中提及成员时使用群名片，不直接展示 QQ 号。
...
export function normalizeGroupInsightAnalysis(parsed, { aiInput, stats }) {
  ...
  const sampleContentMap = new Map(
    (aiInput?.samples || []).map((item) => [
      String(item.reference || "").toUpperCase(),
      item.content,
    ])
  ...
  const quotes = (Array.isArray(source.quotes) ? source.quotes : [])
    .map((quote) => {
      ...
      return {
        ...
        content: safeString(sampleContentMap.get(messageRef), 400)
          || normalizeTranscriptContent(record),
