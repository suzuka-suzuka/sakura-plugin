...
export async function saveConversationHistory(e, currentFullHistory, profilePrefix) {
  ...
  // 裁剪后确保开头不是孤立的 function 响应或含 functionCall 的 model 消息
  // 这些如果没有对应的上下文会导致 API 报错
  while (currentFullHistory.length > 0) {
    ...
    if (first.role === "function") {
      ...
      first.role === "model" &&
      first.parts?.some((p) => p.functionCall)
    ) {
      // 含 functionCall 的 model 消息，其后续 function 响应可能已被裁掉，移除
      currentFullHistory.shift();
    } else {
      break;
    }
