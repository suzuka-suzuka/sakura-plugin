...
async function getChatHistoryGroup(e, num) {
  ...
  try {
    ...
    const allowedMessageTypes = ["text", "at", "image", "video", "bface"]
    ...
    const processChats = rawChats => {
      ...
      return formatted.filter(chat => {
        const messageId = chat.seq || chat.message_id
    ...
    while (chats.length < num) {
      ...
      const newSeq = chatHistory[0]?.seq || chatHistory[0]?.message_id
    ...
    for (const msgPart of originalMsg.message) {
      switch (msgPart.type) {
        ...
        case "image": {
          ...
          const isAnimated = msgPart.asface === true || msgPart.subType === 1
        ...
        case "video": {
          if (adapter === 0) {
            let url
            try {
              url = await e.group.getVideoUrl(msgPart)
            } catch (err) {
              url = await e.group.getNTVideoUrl(msgPart)
            }
            if (url) {
              contentParts.push(`[视频URL:${url}]`)
            } else {
    ...
    for (const msgPart of messageContentParts) {
      switch (msgPart.type) {
        ...
        case "image": {
          const imageUrl = msgPart.url
          const isAnimated = msgPart.asface === true || msgPart.subType === 1
          contentParts.push(isAnimated ? `[动画表情URL:${imageUrl}]` : `[图片URL:${imageUrl}]`)
          break
        }
        case "video": {
          if (adapter === 0) {
