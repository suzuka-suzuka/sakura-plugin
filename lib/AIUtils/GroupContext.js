...
async function formatChatMessageContent(e, chat) {
  ...
  const memberInfo = e.bot.gml.get(e.group_id)?.get(senderId)
  ...
  if (replyPart && replyPart.id) {
    try {
      ...
      if (originalMsg && originalMsg.message) {
        ...
        const originalMemberInfo = e.bot.gml.get(e.group_id)?.get(originalSenderId)
...
export async function buildGroupPrompt(e) {
  ...
  if (e.isGroup) {
    ...
    const botMemberInfo = e.bot.gml.get(e.group_id)?.get(botUin)
    const botName = botMemberInfo?.card || botMemberInfo?.nickname
  ...
  logger.info(systemPromptWithContext)
