...
export async function buildGroupPrompt(groupId, options = {}) {
  ...
  if (promptHeader) {
    ...
    let botMemberInfo;
    try {
      botMemberInfo = await group.getMemberInfo(bot.selfId, false);
    } catch {}
    const botName =
      botMemberInfo?.card || botMemberInfo?.nickname || bot.selfId;

    systemPromptWithContext += `你目前正在一个QQ群聊中。`;
    systemPromptWithContext += `\n群名称: ${groupInfo.group_name}, 群号: ${groupId}。`;
    systemPromptWithContext += `你现在是这个QQ群的成员，你的昵称是“${botName}”(QQ:${bot.selfId})。`;
