...
const KEY_PREFIX = "sakura:group-message-history:v1";
...
export function createGroupMessageRecord(event, { nowMs = Date.now() } = {}) {
  if (!event?.group_id) return null;

  const time = normalizeTimestampSeconds(event.time, nowMs);
  ...
  const messageId = getMessageIdentifier(
    ...
    event.real_id,
    event.real_seq
  ) || `${time}:${userId}:${nowMs}:${Math.random().toString(36).slice(2)}`;
...
export async function appendGroupMessage(
  ...
  const record = createGroupMessageRecord(event, { nowMs });
  ...
  const score = record.time * 1000;
  const cutoff = nowMs - GROUP_MESSAGE_TTL_SECONDS * 1000;
...
function parseStoredRecord(payload) {
  if (!payload) return null;
  try {
    const record = JSON.parse(payload);
    if (!record || typeof record !== "object") return null;

    const messageId = getMessageIdentifier(record.messageId, record.messageSeq);
    const messageSeq = getMessageIdentifier(record.messageSeq, messageId);
    return {
      ...record,
      messageId,
      messageSeq,
      realSeq: normalizeMessageIdentifier(record.realSeq),
    };
  } catch {
    return null;
  }
...
export async function getRecentGroupMessages({
  ...
  for (let offset = 0; offset < maxScan && recentRecords.length < resultLimit;) {
    ...
    for (const payload of payloads || []) {
      const record = parseStoredRecord(payload);
  ...
  replyIds.forEach((id, index) => {
    const replyRecord = parseStoredRecord(replyPayloads?.[index]);
...
export async function getGroupMessagesByTimeRange({
  ...
  const ids = await redisClient.zrangebyscore(
    timelineKey,
    String(Math.floor(normalizedStartTime * 1000)),
    String(Math.floor(normalizedEndTime * 1000)),
    "LIMIT",
    0,
    resultLimit
  );
...
export async function getActiveRecordedGroups({
  ...
  return groupIds.map((groupId, index) => {
    const stored = parseStoredRecord(payloads?.[index]);
    return {
