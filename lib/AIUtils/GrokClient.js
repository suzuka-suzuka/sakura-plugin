...
async function downloadMedia(page, url, savePath, type = "image", config = {}) {
  ...
  try {
    ...
    await ensureBrowserCookies(downloadPage, config, [
      "https://grok.com",
      "https://assets.grok.com",
    ])

    const downloadTimeout = type === "video" ? 180000 : 60000
    ...
    downloadPage.on("response", async response => {
      if (response.url() === resolvedUrl) {
        try {
          ...
          rejectResponse(new Error(`读取响应失败: ${e.message}`))
        }
      }
    })
    ...
    const buffer = await Promise.race([
      ...
      new Promise((_, reject) =>
        setTimeout(
          () => reject(new Error(`下载超时 (${type === "video" ? "3分钟" : "1分钟"})`)),
          downloadTimeout,
        ),
      ),
    ])
...
async function processResponse(page, data, config, e) {
  ...
  if (message.includes("**Search Results:**")) {
    ...
    if (searchResults.length > 0 && e) {
      const forwardMessages = searchResults.map((item, index) => {
        return {
          text: `【${index + 1}】${item.title}\n链接：${item.url}\n摘要：${item.preview}`,
          senderId: e.self_id || e.bot?.uin || 2854196310,
          senderName: "搜索结果",
        }
      })

      await makeForwardMsg(e, forwardMessages, `搜索结果 (${searchResults.length}条)`)
    }
    ...
    for (const line of lines) {
      try {
        ...
        if (videoResp?.videoUrl) {
          ...
          logger.info("[Grok] 发现视频URL:", absoluteUrl)
        }
