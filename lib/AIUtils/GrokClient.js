...
let grokStatsigChunkSourcePromise = null
...
async function resetBrowserInstance(reason = "") {
  ...
  try {
    ...
    logger.warn(`[Grok] close stale browser failed${reason ? ` (${reason})` : ""}: ${error.message}`)
  }
}
...
function isAntiBotRejection(error) {
  const message = String(error?.message || error || "").toLowerCase()
  return message.includes("anti-bot") || (message.includes("http 403") && message.includes("request rejected"))
}
...
async function loadGrokStatsigChunkSource() {
  if (!grokStatsigChunkSourcePromise) {
    grokStatsigChunkSourcePromise = axios
      .get(GROK_STATSIG_CHUNK_URL, { responseType: "text", timeout: 30000 })
      .then(response => String(response.data || ""))
  }
  return await grokStatsigChunkSourcePromise
}
...
async function generateBrowserStatsigId(page, pathname, method = "POST") {
  ...
  try {
    const chunkSource = await loadGrokStatsigChunkSource()
    if (!chunkSource.includes("TURBOPACK")) {
      throw new Error("statsig chunk did not return JavaScript")
    }

    return await page.evaluate(
      async ({ source, pathname, method }) => {
        window.__sakuraGrokStatsigSignerPromise ||= (async () => {
          const previousTurbopack = window.TURBOPACK
          let defaultExport = null
          window.TURBOPACK = {
            push(payload) {
              for (let i = 1; i < payload.length; i += 2) {
                const moduleFactory = payload[i + 1]
          ...
          if (typeof defaultExport !== "function") {
            throw new Error("statsig default export not found")
          }
          const signer = defaultExport()
          if (typeof signer !== "function") {
            throw new Error("statsig signer not found")
          }
          return signer
        })()

        const signer = await window.__sakuraGrokStatsigSignerPromise
        try {
      ...
      { source: chunkSource, pathname: cleanPathname, method: cleanMethod },
    )
  } catch (error) {
    logger.warn(`[Grok] 生成浏览器 statsig 失败，使用旧算法: ${error.message}`)
...
async function sendRequest(page, payload, config, options = {}) {
  ...
  throw new Error("Grok request retry failed")
}
...
async function executeGrokRequest(request, config, e) {
  ...
  try {
    ...
    if (isVideoModel) {
      ...
      const videoMode = assetUrls.length > 0 && !prompt ? "normal" : DEFAULT_VIDEO_MODE
