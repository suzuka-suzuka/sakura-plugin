...
const GROK_VIDEO_CHAT_MODEL = "grok-3"
const GROK_VIDEO_CHAT_MODEL_MODE = "MODEL_MODE_GROK_3"
...
function parseGrokVideoOptions(input = "") {
  ...
  for (const token of tokens) {
    ...
    if (/^(720p|1080p|hd|draft|standard)$/.test(lower)) {
      quality = lower
    ...
    if (/^(720x1280|1080x1920|720x720|1080x1080|1280x720|1920x1080)$/.test(lower)) {
      size = lower
      if (lower === "720x1280" || lower === "1080x1920") {
        aspectRatio = "9:16"
      } else if (lower === "720x720" || lower === "1080x1080") {
        aspectRatio = "1:1"
      } else {
        aspectRatio = "16:9"
      }
...
function videoConfig(size, aspect, quality) {
  ...
  let resolvedSize = String(size || DEFAULT_VIDEO_SIZE).trim() || DEFAULT_VIDEO_SIZE

  if (resolvedAspect === "auto") {
    const fallbackWidth = resolvedResolution === "720p" ? 1280 : 1920
    const fallbackHeight = resolvedResolution === "720p" ? 720 : 1080
  ...
  const parts = resolvedSize.split("x").map(v => parseInt(v, 10))
  const width = Number.isFinite(parts[0]) ? parts[0] : 1920
  const height = Number.isFinite(parts[1]) ? parts[1] : 1080
...
async function createMediaPost(page, config, { mediaType, mediaUrl = "", prompt = "" } = {}) {
  try {
    ...
    throw lastError || new Error("media post create failed")
  } catch (error) {
...
function buildVideoPayload({
  ...
  const payload = buildPayload(message, modelName, GROK_VIDEO_CHAT_MODEL_MODE, fileAttachments, true, false)
  payload.modelMode = DEFAULT_VIDEO_MODE
  ...
  if (aspectRatio) {
    payload.responseMetadata.modelConfigOverride.modelMap.videoGenModelConfig.aspectRatio = aspectRatio
  }

  if (imageReferences.length > 0) {
    payload.responseMetadata.modelConfigOverride.modelMap.videoGenModelConfig.isReferenceToVideo =
      isReferenceToVideo || imageReferences.length > 0
    payload.responseMetadata.modelConfigOverride.modelMap.videoGenModelConfig.imageReferences = imageReferences
  }
...
async function fetchMediaPost(page, postId, config) {
  ...
  const headers = {
    ...getDynamicHeaders(config, "/rest/media/post/get"),
  }
  ...
  try {
    const result = await page.evaluate(
      ...
      { url: POST_GET_ENDPOINT, headers, body: { id: cleanPostId } },
    )
...
async function sendRequest(page, payload, config, options = {}) {
  try {
    ...
    if (options.conversationId && options.parentResponseId) {
      headers["Referer"] = `https://grok.com/c/${options.conversationId}?rid=${options.parentResponseId}`
    }
    ...
    const responseText = await page.evaluate(
      ...
      { url: requestUrl, headers, body: payload },
    )
...
async function executeGrokRequest(request, config, e) {
  ...
  try {
    ...
    if (isVideoModel) {
      ...
      const fallbackPrompt = prompt || "Generate a video"
      let parentPostId = ""
      let videoMessage = ""
      let fileAttachments = []
      ...
      if (assetUrls.length === 1) {
        ...
        videoMessage = `${assetUrls[0]}  ${fallbackPrompt} --mode=${DEFAULT_VIDEO_MODE}`
      } else {
        parentPostId = await createMediaPost(page, config, {
        ...
        if (assetUrls.length > 1) {
          fileAttachments = fileIds.filter(Boolean)
          const mentions = fileAttachments.map(fileId => `@${fileId}`).join(" ")
          videoMessage = `${mentions} ${fallbackPrompt} --mode=${DEFAULT_VIDEO_MODE}`.trim()
        } else {
          videoMessage = `${fallbackPrompt} --mode=${DEFAULT_VIDEO_MODE}`
        }
      }
      ...
      const result = await sendRequest(page, payload, config, { refererId: parentPostId, e })
