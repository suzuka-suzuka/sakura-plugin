...
const API_DISPLAY_NAME = "OpenAI-compatible media API";
...
async function apiFetch(endpoint, { method = "POST", body = null } = {}, rawConfig) {
  ...
  if (!response.ok) {
    ...
    throw new Error(
      `${API_DISPLAY_NAME} ${method} ${endpoint} failed: ${response.status} ${message.slice(0, 500)}`
    );
  }
...
export async function generateGrokImage(options = {}, rawConfig = {}) {
  ...
  if (buffers.length === 0) {
    throw new Error(`${API_DISPLAY_NAME} did not return image output`);
  }
...
export async function createGrokVideo(options = {}, rawConfig = {}) {
  ...
  if (!prompt && !native) {
    throw new Error("prompt is required when using the /videos wrapper");
  }
...
export async function generateGrokVideoAndWait(options = {}, rawConfig = {}) {
  ...
  if (!created.requestId) {
    throw new Error(`${API_DISPLAY_NAME} did not return a video request id`);
  }
