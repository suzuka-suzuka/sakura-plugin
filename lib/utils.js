...
<body id="md-body">${htmlBody}</body></html>`;
    ...
    if (bodyHeight > currentWidth * 2) {
      ...
      // 最宽可以允许到 1620，超过的就算了，也能保证适度
      targetWidth = Math.min(targetWidth, 1620);

      if (targetWidth > currentWidth) {
        currentWidth = targetWidth;
        await page.evaluate((w) => {
          document.body.style.maxWidth = `${w}px`;
          // 轻微补偿字号（按比例开方），不完全按比例放大以降低高度，又能保证缩放后有较好可读性
          const scale = w / 1080;
          const newFontSize = Math.round(34 * Math.sqrt(scale));
