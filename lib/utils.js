...
const MARKDOWN_IMAGE_WIDTH = 1080;
const MARKDOWN_IMAGE_MAX_HEIGHT = MARKDOWN_IMAGE_WIDTH * 2;
const MARKDOWN_IMAGE_MIN_SLICE_HEIGHT = 300;
...
export async function sendMarkdownMsg(e, markdownContent, opts = {}) {
  ...
  if (content.length === 0) {
    const imgBuffers = await renderMarkdownToImages(markdownContent).catch(() => []);
    if (imgBuffers.length > 0) {
      content = imgBuffers.map((imgBuffer) => segment.image(imgBuffer));
    } else {
...
export async function smartReplyMsg(e, text, opts = {}) {
  ...
  if (isMdText(text)) {
    ...
    try {
      let fallbackContent = [];
      imgBuffers = await renderMarkdownToImages(text).catch(() => []);
      if (imgBuffers.length > 0) {
        fallbackContent.push(...imgBuffers.map((imgBuffer) => segment.image(imgBuffer)));
      } else {
    ...
    if (imgBuffers.length > 0) {
      return await e.reply(imgBuffers.map((imgBuffer) => segment.image(imgBuffer)), quote, at);
    }
  }
...
export async function renderMarkdownToImage(markdownText) {
  const imageBuffers = await renderMarkdownToImages(markdownText);
  return imageBuffers[0] || null;
}

export async function renderMarkdownToImages(markdownText) {
  let browser = null;
  try {
    const mathBlocks = [];
...
<body id="md-body">${htmlBody}</body></html>`;
    ...
    await page.setViewport({ width: MARKDOWN_IMAGE_WIDTH, height: 100, deviceScaleFactor: 2 });
    ...
    await page.setViewport({ width: MARKDOWN_IMAGE_WIDTH, height: Math.ceil(bodyHeight), deviceScaleFactor: 2 });
    const slices = await page.evaluate((maxHeight, minHeight, imageWidth) => {
      const fullHeight = Math.ceil(document.body.scrollHeight);
      const selectors = [
        "h1", "h2", "h3", "h4", "h5", "h6",
        "p", "blockquote", "ul", "ol", "hr",
        "pre", "table", ".mermaid", "mjx-container[display='true']",
      ];
      const blocks = [...document.body.querySelectorAll(selectors.join(","))]
        .map((el) => {
          const rect = el.getBoundingClientRect();
          return {
            top: Math.max(0, Math.floor(rect.top + window.scrollY)),
            bottom: Math.min(fullHeight, Math.ceil(rect.bottom + window.scrollY)),
            unbreakable: el.matches("pre, table, .mermaid"),
          };
        })
      ...
      while (startY < fullHeight) {
        const targetEnd = Math.min(startY + maxHeight, fullHeight);
        let endY = targetEnd;

        if (targetEnd < fullHeight) {
          const crossingUnbreakable = blocks.find((block) =>
            block.unbreakable &&
            block.top < targetEnd &&
          ...
          if (crossingUnbreakable) {
            if (crossingUnbreakable.top - startY >= minHeight) {
              endY = crossingUnbreakable.top;
            } else {
              endY = crossingUnbreakable.bottom;
            }
          } else {
            const breakpoint = blocks
              .map((block) => block.bottom)
              .filter((bottom) => bottom > startY + minHeight && bottom <= targetEnd)
              .pop();
        ...
        if (endY <= startY) {
          endY = Math.min(startY + maxHeight, fullHeight);
        }
      ...
      return result;
    }, MARKDOWN_IMAGE_MAX_HEIGHT, MARKDOWN_IMAGE_MIN_SLICE_HEIGHT, MARKDOWN_IMAGE_WIDTH);

    const imageBuffers = [];
    for (const slice of slices) {
      imageBuffers.push(await page.screenshot({
        type: "png",
        clip: slice,
      }));
    }
