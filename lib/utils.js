 ...
 *
 * 优先级：
 *   ≥600 字 + Markdown  → 三层嵌套转发（Markdown 渲染）+ 第一层附带降级节点（图片或 Unicode 文本），失败后继续往下
 *   ≥300 字 + 复杂 Md   → Puppeteer 渲染图片直接发送，失败则降级纯文本
 *   ≥300 字 + 简单 Md   → Unicode 美化文本后走 textReplyFn / e.reply
 *   其他                → 原样走 textReplyFn / e.reply
 *
 * @param {object}   e              - 消息事件对象
...
export async function smartReplyMsg(e, text, opts = {}) {
  ...
  // ── ≥600 字：三层嵌套转发 + 同层降级节点 ──────────────────────────────
  if (isMdText(text) && text.length >= 600) {
    try {
      let fallbackContent;
      if (hasComplexMd(text)) {
        const imgBuffer = await renderMarkdownToImage(text).catch(() => null);
        if (imgBuffer) fallbackContent = [segment.image(imgBuffer)];
      }
      if (!fallbackContent) {
        fallbackContent = [{ type: "text", data: { text: mdToPlainText(text) } }];
      }
  ...
  // ── ≥300 字：按内容特征分流 ───────────────────────────────────────────
  if (isMdText(text) && text.length >= 300) {
    if (hasComplexMd(text)) {
      try {
        const imgBuffer = await renderMarkdownToImage(text);
        if (imgBuffer) return await e.reply(segment.image(imgBuffer), quote, at);
        logger.warn("[smartReplyMsg] Markdown图片渲染失败，降级");
      } catch (err) {
        logger.error(`[smartReplyMsg] Markdown图片渲染出错: ${err.message}，降级`);
      }
    } else {
      text = mdToPlainText(text);
    }
  }
...
/**
 * 检测文本是否含有「复杂」Markdown（代码块或表格）
 * 触发时应渲染为图片而非纯文本
 * @param {string} text
 * @returns {boolean}
 */
export function hasComplexMd(text) {
  return /```[\s\S]*?```|^\|.+\|/m.test(text);
}

/**
 * 将 Markdown 转换为 Unicode 美化纯文本（适用于仅含标题/列表等简单结构）
 * @param {string} text
 * @returns {string}
 */
export function mdToPlainText(text) {
  return text
    // 标题
...
<html><head><meta charset="utf-8"><style>
  * { box-sizing: border-box; }
  ...
  strong { color: #f38ba8; }
  em { color: #fab387; }
  hr { border: none; border-top: 1px solid #45475a; margin: 0.8em 0; }
</style></head>
<body id="md-body">${htmlBody}</body></html>`;
    ...
    await page.setViewport({ width: 740, height: 100 });
    await page.setContent(fullHtml, { waitUntil: "networkidle0" });

    // 按内容实际高度截图，避免留白
    const bodyHeight = await page.evaluate(() => document.body.scrollHeight);
    await page.setViewport({ width: 740, height: bodyHeight });
