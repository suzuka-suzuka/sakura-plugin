...
export function isMdText(text) {
  ...
  return hasCodeBlock || hasTable || hasHeading || hasOrderedList || hasUnorderedList || hasBlockquote || hasBold || hasLatex;
}
...
<style>
  * { box-sizing: border-box; }
  body {
    ...
    font-size: 34px; /* 再次调大字体 */
    line-height: 1.8;
    max-width: 750px; /* 限制大宽度，迫使图片在缩放时文字更大 */
  }
  h1, h2, h3, h4, h5, h6 {
    color: #cba6f7;
...
<body id="md-body">${htmlBody}</body></html>`;
    ...
    await page.setViewport({ width: 750, height: Math.ceil(bodyHeight), deviceScaleFactor: 2 });
