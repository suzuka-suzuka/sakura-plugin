...
export default class FishingImageGenerator extends EconomyImageGenerator {
  ...
  async generateFishingRank(ranking) {
      const width = 800
      const itemHeight = 100
      const headerHeight = 120
      const padding = 20
      const height = headerHeight + ranking.length * (itemHeight + padding) + padding
      ...
      // Header
      ctx.fillStyle = '#5D4037'
      ctx.font = `bold 45px ${this.fontFamily}`
      ctx.textAlign = 'center'
      ctx.fillText('🏆 钓鱼排行榜 TOP10', width / 2, 75)
      ctx.textAlign = 'left'
      ...
      for(let i=0; i<ranking.length; i++) {
          ...
          // Background
          ctx.fillStyle = 'rgba(255, 255, 255, 0.6)'
          this.drawRoundedRect(ctx, x, y, width - 2 * padding, itemHeight, 15)
          ctx.fill()
          ...
          ctx.textAlign = 'center'
          ctx.fillText(`#${i+1}`, x + 40, y + 65)
          ctx.textAlign = 'left'
          
          // Avatar
          const avatarUrl = `https://q1.qlogo.cn/g?b=qq&nk=${user.userId}&s=100`
          await this.drawAvatar(ctx, avatarUrl, x + 90, y + 10, 80)
          
          // Name
          ctx.fillStyle = '#5D4037'
          ctx.font = `bold 28px ${this.fontFamily}`
          ctx.fillText(this.truncateText(ctx, user.nickname, 300), x + 190, y + 45)
          
          // Stats
          ctx.font = `22px ${this.fontFamily}`
          ctx.fillStyle = '#795548'
          ctx.fillText(`💰 ${user.coins}`, x + 190, y + 80)
          ctx.fillText(`🎣 ${user.catchCount} 次`, x + 450, y + 80)
      }
