...
export default class FishingImageGenerator extends EconomyImageGenerator {
  ...
  async generateFishingRecord(userData, history, targetName, targetId) {
    ...
    const headerHeight = 280  // 增加高度以容纳鱼雷统计
    ...
    // 鱼雷统计
    const torpedoStats = userData.torpedoStats || { deployed: 0, detonated: 0, hitByOthers: 0, hitOthers: 0 }
    ctx.font = `24px ${this.fontFamily}`
    ctx.fillStyle = '#D32F2F'
    ctx.fillText(`💣 鱼雷投放：${torpedoStats.deployed} | 引爆：${torpedoStats.detonated} | 钓到雷：${torpedoStats.hitOthers} | 被钓：${torpedoStats.hitByOthers}`, 40, 220)
