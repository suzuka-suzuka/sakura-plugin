...
export default class FishingImageGenerator extends EconomyImageGenerator {
  ...
  async generateFishingRecord(userData, history, targetName, targetId) {
    ...
    ctx.fillText(`🎣 总钓鱼次数：${userData.totalCatch || 0} 次`, 200, 125)
    ctx.fillText(`💰 总收益：${userData.totalEarnings || 0} 樱花币`, 200, 160)
    ctx.fillText(`💥 被炸次数：${userData.torpedoHits || 0} 次`, 200, 195)
