...
export default class EconomyImageGenerator {
  ...
  async generateTransferImage(data) {
    ...
    ctx.textAlign = "center"
    const transferText = `转账 ${data.amount} 樱花币`
    const transferTextWidth = ctx.measureText(transferText).width
    ctx.fillText(transferText, this.width / 2, centerY - 20)
    
    if (this.coinIcon) {
        this.drawCoinIcon(ctx, this.width / 2 + transferTextWidth / 2 + 10, centerY - 45, 45)
    }
