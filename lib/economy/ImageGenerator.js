...
const coinIconPath = path.join(pluginresources, "economy", "coin", "coin.png")
...
export default class EconomyImageGenerator {
  ...
  async drawCoinIcon(ctx, x, y, size = 32) {
    try {
        const img = await loadImage(coinIconPath)
        ctx.drawImage(img, x, y, size, size)
    } catch (e) {
        // 如果图标加载失败，绘制一个简单的圆形图标
        ctx.fillStyle = "#FF69B4"
        ctx.beginPath()
  ...
  async generateStatusImage(data) {
    ...
    const drawStat = async (label, value, x, y, color, showIcon = false) => {
        ...
        if (showIcon) {
            ...
            await this.drawCoinIcon(ctx, x + textWidth + 10, y + 20, 28)
        } else {
            ctx.fillText(value, x, y + 45)
        }
    }

    drawStat("当前等级", `Lv.${data.level}`, 50, 240, "#FF69B4", false)
    await drawStat("樱花币", `${data.coins}`, 300, 240, "#FF1493", true)
    drawStat("当前经验", `${data.experience}`, 550, 240, "#FFB3D9", false)
  ...
  async generateRankingImage(data) {
    ...
    for (let i = 0; i < data.list.length; i++) {
        ...
        // 如果是樱花币排行榜，添加图标
        if (data.title.includes("樱花币")) {
            await this.drawCoinIcon(ctx, this.width - 50 - valueWidth - 35, y + 42, 28)
        }
  ...
  async generateTransferImage(data) {
    ...
    ctx.fillText(transferText, this.width / 2 - 20, centerY - 20)
    await this.drawCoinIcon(ctx, this.width / 2 + transferTextWidth / 2 - 15, centerY - 36, 32)
    ctx.textAlign = "left"
    ...
    ctx.fillText(senderCoinText, leftX - senderCoinWidth / 2, centerY + avatarSize/2 + 95)
    await this.drawCoinIcon(ctx, leftX + senderCoinWidth / 2 + 5, centerY + avatarSize/2 + 78, 24)
    
    ctx.fillText(receiverCoinText, rightX - receiverCoinWidth / 2, centerY + avatarSize/2 + 95)
    await this.drawCoinIcon(ctx, rightX + receiverCoinWidth / 2 + 5, centerY + avatarSize/2 + 78, 24)
