...
class ShopManager {
  ...
  async buyItem(e, itemName, count = 1) {
    ...
    // 处理实际购买数量与请求数量不同的情况（如鱼饵库存上限）
    const actualCount = result.actualCount || count;
    if (actualCount < count) {
      // 退还多扣的钱
      const refund = (count - actualCount) * item.price;
      economyManager.addCoins(e, refund);
      
      return {
        success: true,
        msg: `🎉 购买成功！\n由于鱼饵袋空间限制，实际购买了 ${actualCount} 个【${item.name}】\n💰 花费：${actualCount * item.price} 樱花币（已退还 ${refund} 樱花币）`,
        item,
        count: actualCount,
      };
    }
  ...
  async handlePurchase(e, item, count) {
    switch (item.handler) {
      case "gift":
        return this.handleGiftPurchase(e, item, count);
      case "fishing_rod":
        return this.handleFishingRodPurchase(e, item);
      case "fishing_bait":
        return this.handleFishingBaitPurchase(e, item, count);
      case "buff":
        return this.handleBuffPurchase(e, item);
      default:
        return this.handleDefaultPurchase(e, item, count);
    }
  }
  ...
  async checkEquipmentOwned(e, item) {
    switch (item.handler) {
      case "fishing_rod":
        ...
        return fishingManager.hasRod(e.user_id, item.id);
      default:
        return false;
    }
  }
  ...
  handleGiftPurchase(e, item, count) {
    this.addToInventory(e.group_id, e.user_id, item.name, count);
    return { success: true };
  }

  handleFishingRodPurchase(e, item) {
    const fishingManager = new FishingManager(e.group_id);
    fishingManager.buyRod(e.user_id, item.id);
    return { success: true };
  }
  ...
  handleFishingBaitPurchase(e, item, count) {
    ...
    // 检查鱼饵库存上限（所有鱼饵总数最多5个）
    const maxBaitTotal = 5;
    const userData = fishingManager.getUserData(e.user_id);
    const currentBaitTotal = Object.values(userData.baits || {}).reduce((sum, c) => sum + c, 0);
    
    if (currentBaitTotal >= maxBaitTotal) {
      return {
        success: false,
        msg: `你的鱼饵袋已满（${currentBaitTotal}/${maxBaitTotal}），先去钓鱼消耗一些吧~`
      };
    }
    
    // 自动调整购买数量为可用空间
    const availableSpace = maxBaitTotal - currentBaitTotal;
    const actualCount = Math.min(count, availableSpace);
    
    fishingManager.buyBait(e.user_id, item.id, actualCount);
    
    // 返回实际购买数量，用于计算退款
    return { success: true, actualCount, requestedCount: count };
  }
