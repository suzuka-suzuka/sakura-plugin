...
export default class FishingManager {
  ...
  _initUser(userId) {
    if (!this.data[userId]) {
      this.data[userId] = {
        ...
        rodProficiency: {}, // 鱼竿熟练度 {rodId: proficiency}
    ...
    if (!this.data[userId].rodProficiency) {
      this.data[userId].rodProficiency = {};
    }
  ...
  // 清除指定鱼竿的熟练度记录
  clearRodProficiency(userId, rodId) {
    const userData = this._initUser(userId);
    if (userData.rodProficiency && userData.rodProficiency[rodId]) {
      delete userData.rodProficiency[rodId];
      this._save();
    }
  }
  ...
  // 获取鱼竿熟练度
  getRodProficiency(userId, rodId = null) {
    const userData = this._initUser(userId);
    const targetRodId = rodId || userData.rod;
    if (!targetRodId) return 0;
    return userData.rodProficiency?.[targetRodId] || 0;
  }

  // 增加鱼竿熟练度
  addRodProficiency(userId, rodId = null) {
    const userData = this._initUser(userId);
    const targetRodId = rodId || userData.rod;
    if (!targetRodId) return 0;
    
    if (!userData.rodProficiency) {
      userData.rodProficiency = {};
    }
    if (!userData.rodProficiency[targetRodId]) {
      userData.rodProficiency[targetRodId] = 0;
    }
    userData.rodProficiency[targetRodId]++;
    this._save();
    return userData.rodProficiency[targetRodId];
  }
  ...
  canDetonateTorpedo(userId) {
    ...
    const now = Date.now();
    const canDetonateTime = torpedo.deployTime + 12 * 60 * 60 * 1000;
    if (now < canDetonateTime) {
      const remainingMs = canDetonateTime - now;
