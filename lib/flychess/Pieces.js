...
class Piece {
  ...
  _testState(allPieces, R, r) {
    ...
    if (this.position.color === this.color && this.position.s) {
      ...
      const nextPos = this._getNextColor(allPieces, R)
      if (nextPos) {
    ...
    } this.position = this._getNextColor(allPieces, R)
      R.tipsJump()
      this._tryAttack(allPieces, R, r)
