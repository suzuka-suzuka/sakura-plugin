...
class GameManager {
  ...
  async movePiece(side, piece, pieceIndex) {
    ...
    if (!piece.isReady) {
      ...
      const ruleCallbacks = {
        ...
        tipsJump: async () => await this.context.sendMsg("🚀 踩中同色格子，向前跳跃！"),
        tipsFly: async () => await this.context.sendMsg("✈️ 触发飞行航线，超级飞行！"),
        tipsBlock: async () => await this.context.sendMsg("🚧 前方有叠子阻挡，无法通过！"),
  ...
  async processTurn(isExtraTurn = false) {
    ...
    if (side.stats) {
      side.stats.diceRolls[this.lastDiceRoll] = (side.stats.diceRolls[this.lastDiceRoll] || 0) + 1
    }
  ...
  async rollbackAllPieces(side) {
    ...
    await this.context.sendMsg([segment.at(side.q), " 不好！你连续掷出了3个6，所有棋子都返回了停机坪！"])
  }
  ...
  async checkPlayerWin(side) {
    ...
    if (side.pieces.every(p => p.win)) {
      ...
      await this.context.sendMsg(segment.at(side.q), ` 的所有棋子都已到达终点，获得了胜利！`)
  ...
  async endGame() {
    ...
    for (const side of this.game.sides) {
      let memberInfo
      try {
        memberInfo = await this.context.pickMember(side.q)?.getInfo(true)
      } catch {
        try {
          memberInfo = (await this.context.pickMember(Number(side.q))).info
        } catch {}
      }
