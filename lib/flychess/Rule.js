...
class GameManager {
  ...
  async movePiece(side, piece, pieceIndex) {
    ...
    if (!piece.isReady) {
      ...
      const ruleCallbacks = {
        attack: (isStackCollision, opponents) => {
          ...
          if (isStackCollision) {
            this.context.sendMsg("💥 叠子碰撞！双方棋子都返回了停机坪！")
          } else {
        ...
        tipsBlock: () => this.context.sendMsg("🚧 前方有叠子阻挡，无法通过！"),
  ...
  async endGame() {
    ...
    drawStats(this.game, nicknames).then(buffer => {
      if (buffer) {
        this.context.sendMsg(segment.image(buffer))
      }
    })
