...
class GameManager {
  ...
  getMovablePieces(side, diceRoll) {
    ...
    return side.pieces
      ...
      .filter((item) => {
        ...
        if (item.piece.isReady) return !item.piece.isReadyDoorBlocked(allPieces, diceRoll);
  ...
  exportState() {
    ...
    return {
      ...
      manager: {
        ...
        winRanking: [...this.winRanking],
      },
      game: {
        turnIndex: this.game.turnIndex,
        currentSideColor: this.game.currentSide?.color || null,
        currentSidePlayerId: this.game.currentSide?.q || null,
        sides: this.game.sides.map((side) => ({
          color: side.color,
          q: side.q,
          win: side.win,
          up0: side.up0,
          up1: side.up1,
          stats: JSON.parse(JSON.stringify(side.stats || {})),
          pieces: side.pieces.map((piece) => ({
            id: piece.id,
            positionId: piece.position?.id,
            isReady: piece.isReady,
            win: piece.win,
            isStacked: piece.isStacked,
            roadIndex: piece.road?.index ?? 0,
          })),
        })),
      },
    };
  ...
  restoreState(state) {
    if (!state?.game?.sides?.length) {
      throw new Error("存档数据不完整。");
    }

    const game = new Game();

    for (const sideState of state.game.sides) {
      if (!sideState?.q || !sideState?.color) {
        throw new Error("存档玩家数据不完整。");
      }

      if (!game.addPlayer(sideState.q, sideState.color)) {
        throw new Error(`无法恢复 ${sideState.color} 方玩家。`);
      }

      const side = game.sides[game.sides.length - 1];
      side.win = Boolean(sideState.win);
      side.up0 = Number.isFinite(sideState.up0) ? sideState.up0 : 0;
      side.up1 = Number.isFinite(sideState.up1) ? sideState.up1 : 0;
      side.stats = {
        ...side.stats,
        ...sideState.stats,
        diceRolls: {
          ...side.stats.diceRolls,
          ...(sideState.stats?.diceRolls || {}),
        },
        attacks: {
          ...side.stats.attacks,
          ...(sideState.stats?.attacks || {}),
        },
      };

      side.pieces.forEach((piece, index) => {
        const pieceState = sideState.pieces?.[index];
        if (!pieceState) {
          return;
        }

        piece.isReady = Boolean(pieceState.isReady);
        piece.win = Boolean(pieceState.win);
        piece.isStacked = Boolean(pieceState.isStacked);
        piece.position = ID2POSITION.get(pieceState.positionId) || piece.oPosition;

        if (
          Number.isInteger(pieceState.roadIndex) &&
          pieceState.roadIndex >= 0 &&
          pieceState.roadIndex < piece.road.list.length
        ) {
          piece.road.index = pieceState.roadIndex;
        } else {
          piece.road.update(piece.position.id);
        }
      });
    }

    const savedTurnIndex = state.game.turnIndex;
    game.turnIndex =
      Number.isInteger(savedTurnIndex) &&
        savedTurnIndex >= -1 &&
        savedTurnIndex < game.sides.length
        ? savedTurnIndex
        : -1;
    const savedCurrentSide = game.sides.find(
      (side) =>
        side.color === state.game.currentSideColor &&
        side.q === state.game.currentSidePlayerId
    );
    game.currentSide =
      savedCurrentSide || (game.turnIndex >= 0 ? game.sides[game.turnIndex] : null);

    const currentSideIndex = game.sides.indexOf(game.currentSide);
    if (currentSideIndex !== -1) {
      game.turnIndex = currentSideIndex;
    }

    this.game = game;
    this.isStarted = Boolean(state.manager?.isStarted);
    this.startTime = Number.isFinite(state.manager?.startTime)
      ? state.manager.startTime
      : Date.now();
    this.gameState = Number.isInteger(state.manager?.gameState)
      ? state.manager.gameState
      : GAME_STATE.IDLE;
    this.lastDiceRoll = Number.isInteger(state.manager?.lastDiceRoll)
      ? state.manager.lastDiceRoll
      : 0;
    this.winRanking = Array.isArray(state.manager?.winRanking)
      ? [...state.manager.winRanking]
      : [];
  }

  async sendCurrentBoard() {
    if (!this.game) {
      return;
    }

    const imageBuffer = await drawGameBoard(
      this.game,
