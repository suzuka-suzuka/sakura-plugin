...
export async function downloadImage(imageUrl) {
    ...
    try {
        ...
        logger.error(`下载图片时出错: ${error.message}, 状态码: ${error.response ? error.response.status : '未知'}`);
        return false;
    }
}
...
export async function yandeimage(apiUrl) {
    ...
    try {
        ...
        logger.error(` 请求图片 API 时出错: ${error.message}, 状态码: ${error.response ? error.response.status : '未知'}`);
        return false;
    }
}
...
export async function getgif(apiUrl) {
    ...
    try {
        ...
        if (jsonData && typeof jsonData === 'object' && Array.isArray(jsonData.results) && jsonData.results.length > 0) {
            ...
            if (imageUrl) {
                ...
                logger.warn(`成功获取到 API 数据，但在第一个结果项中未找到预期的 media_formats.gif.url 字段。`);
                return false;
            }
        } else {
...
export async function checkAndFlipImage(imageBuffer, sendResult) {
    if (sendResult && sendResult.message_id) {
        return { success: true, processedBuffer: imageBuffer };
    }

    logger.warn('图片首次发送请求失败，尝试翻转。');
    ...
    try {
        ...
        return { success: false, processedBuffer: flippedImageBuffer };
    } catch (sharpError) {
        logger.error('使用 sharp 翻转图片失败:', sharpError);
        return { success: false, processedBuffer: imageBuffer };
    }
}
