...
import { _path, pluginRoot, pluginresources } from "./path.js";
...
class Setting {
  constructor () {
    ...
    this.watcher = { config: {}, defSet: {}, economy: {} };
  }
  ...
  getEconomy (app) {
    return this.getYaml(app, 'economy');
  }
  ...
  setYaml (app, type, objectToWrite){
    ...
    try {
      const currentFileContent = fs.existsSync(file) ? YAML.load(fs.readFileSync(file, 'utf8')) : {};
      if (lodash.isEqual(currentFileContent, objectToWrite)) {
          return false;
      }
      fs.writeFileSync(file, YAML.dump(objectToWrite),'utf8');
      return true;
    } catch (error) {
  ...
  getYaml (app, type, forceRead = false) {
    ...
    try {
      ...
      const parsedYaml = YAML.load(fs.readFileSync(file, 'utf8'));
      if (type !== 'config') {
  ...
  getFilePath (app, type) {
    ...
    if (type === 'economy') return `${this.economyPath}${app}.yaml`;
    else {
