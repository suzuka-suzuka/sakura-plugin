...
class Setting {
  ...
  merge () {
    let sets = {};
    let appsConfig = fs.readdirSync(this.defPath).filter(file => /\.ya?ml$/.test(file));
    for (let appConfig of appsConfig) {
      let filename = appConfig.replace(/\.ya?ml$/, '').trim();
  ...
  getFilePath (app, type) {
    const ext = fs.existsSync(`${this.defPath}${app}.yaml`) ? '.yaml' : '.yml';
    if (type === 'defSet') return `${this.defPath}${app}${ext}`;
    else {
      const configFilePath = `${this.configPath}${app}${ext}`;
      const defFilePath = `${this.defPath}${app}${ext}`;
      try {
