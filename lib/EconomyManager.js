...
export default class EconomyManager {
  ...
  _initUser(e) {
    ...
    if (!this.data[userId]) {
      ...
      // 修复已存在用户数据中缺失或无效的字段
      if (this.data[userId].coins == null || isNaN(this.data[userId].coins)) {
        this.data[userId].coins = 0
      }
      if (this.data[userId].experience == null || isNaN(this.data[userId].experience)) {
        this.data[userId].experience = 0
      }
      if (this.data[userId].level == null || isNaN(this.data[userId].level)) {
  ...
  pay(e, amount) {
    ...
    if (this.config.Groups && Array.isArray(this.config.Groups)) {
      if (!this.config.Groups.includes(Number(this.groupId))) {
        return true
      }
    }
