...
export async function searchGoogleLens(imagePath) {
  // 将浏览器数据存储在插件 data 文件夹下的固定目录中
  const userDataDir = path.resolve(plugindata, 'google-lens-profile')

  // puppeteer-real-browser 需要依赖该目录存在，否则底层写入日志时会报错，所以确保提前创建目录
  ...
  try {
    ...
    // 检查是否有登录按钮，有则等待用户手动登录（给予 60 秒时间）
    const signInButton = await page.$('a[href^="https://accounts.google.com/ServiceLogin"]')
    if (signInButton) {
      console.log('[ImageSearch][GoogleLens] 检测到未登录状态，请在弹出的浏览器窗口中手动登录，程序将等待 60 秒...')
      try {
        await page.waitForFunction(() => !document.querySelector('a[href^="https://accounts.google.com/ServiceLogin"]'), { timeout: 180000 })
        console.log('[ImageSearch][GoogleLens] 登录检测完成，继续执行...')
      } catch (e) {
        console.log('[ImageSearch][GoogleLens] 等待登录超时，将以当前状态继续执行...')
      }
    }
