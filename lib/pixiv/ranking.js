...
async function getRankingLayoutShift(modeKey = '日榜', rankDate = '') {
  try {
    ...
    const shift = Math.min(MAX_LAYOUT_SHIFT_CELLS, Math.max(0, Number(counter - 1) || 0))
    logger.info(`[P站排行榜] 版式偏移检查 mode=${modeKey} day=${dayKey} counter=${counter} shift=${shift}/${MAX_LAYOUT_SHIFT_CELLS}`)
    return shift
  } catch (error) {
...
async function generateRankingImages(illusts, config, modeKey = "日榜", rankDate = '') {
  ...
  const firstItem = layouts?.[0]?.[0]
  if (firstItem) {
    logger.info(`[P站排行榜] 布局检查 mode=${modeKey} shift=${layoutShiftCells} firstRank=#${firstItem.index + 1} row=${firstItem.row} col=${firstItem.col}`)
  }
  const images = []
